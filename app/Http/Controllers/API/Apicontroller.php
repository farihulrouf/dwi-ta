...
class ApiController extends Controller {
    ...
    public function showlogin(Request $request){
        ...
        if ($validator->fails()) {
           ...
           if($getuser){//update token
                 ...
                 $data=AppUser::where("mob_number",$request->get("mobile_no"))->first();
                 if($data){
                      $response['success']="0";
   ...
   public function postregister(Request $request){
        ...
        if ($validator->fails()) {
           ...
           $getuser=AppUser::where("mob_number",$request->get("mobile_no"))->first();
  ...
  public function getsetting($key){
       $getcitydata=Setting::select($key)->find(1);
       ...
       $response = array("success" => "0", "order_details" => "Validation error");
        ...
        if($request->input('payment_type') == "Stripe"){
          ...
          $messages = array(
            ...
            "phone_no.required"=>"phone_no are required"
   ...
   function bookorder(request $request){
        ...
        if ($validator->fails()) {
            $response['order'] = "enter your data perfectly";
               ...
               $order=Order::where("user_id",$request->get("user_id"))->orderby("id",'DESC')->get();
               $status="";
               foreach ($order as $k) {
                    $orderres=OrderResponse::where("set_order_id",$k->id)->get();//get order response
                     if($k->order_status==0){
                  ...
                  if($k->order_status==1&&$k->delivery_mode=='0'){
                                      $status=__('messages.accept');
                  }
                  if($k->order_status==1&&$k->delivery_mode=='1'){
                     $status=__('messages.readyforpickup');
                  }
                  if($k->order_status==2){
                       $status=__('messages.reject');
                  }
                  if($k->order_status==3&&$k->delivery_mode=='0'){
                      $status=__('messages.out_of_delivery');
                  }
                  if($k->order_status==3&&$k->delivery_mode=='1'){
