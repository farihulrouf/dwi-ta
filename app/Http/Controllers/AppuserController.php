...
class AppuserController extends Controller {
  ...
  public function register(Request $request){
       $checkemail=AppUser::where("email",strip_tags(preg_replace('#<script(.*?)>(.*?)</script>#is', '',$request->get("email"))))->first();
       if(isset($checkemail)){
           return __('messages.email already existe');
       }
       else{
               $checkmobile=AppUser::where("mob_number",strip_tags(preg_replace('#<script(.*?)>(.*?)</script>#is', '',$request->get("phone"))))->get();
           	   if(count($checkmobile)==0){
           	   		$store=new AppUser();
                	$store->name=strip_tags(preg_replace('#<script(.*?)>(.*?)</script>#is', '',$request->get("name")));
                	$store->mob_number=strip_tags(preg_replace('#<script(.*?)>(.*?)</script>#is', '',$request->get("phone")));
               		$store->password=md5(strip_tags(preg_replace('#<script(.*?)>(.*?)</script>#is', '',$request->get("password"))));
               		...
               		return 1;
           	   	   ...
           	   	   return __('messages.phone no already existe');
           	   } 
       }
       ...
       if(!Session::get("login_user")){
          return redirect("/");
           ...
           if ($request->hasFile('image')) 
             ...
             $user->name=strip_tags(preg_replace('#<script(.*?)>(.*?)</script>#is', '', $name));
             $user->email=strip_tags(preg_replace('#<script(.*?)>(.*?)</script>#is', '',$email));
             $user->profile_pic=$img_url;
             $user->save();
              Session::put("user_name",strip_tags(preg_replace('#<script(.*?)>(.*?)</script>#is', '', $name)));
              Session::put("user_email",strip_tags(preg_replace('#<script(.*?)>(.*?)</script>#is', '',$email)));
   ...
   public function login(Request $request){
        ...
        $checkdetails=AppUser::where("mob_number",strip_tags(preg_replace('#<script(.*?)>(.*?)</script>#is', '',$request->get("phone"))))->where("password",md5(strip_tags(preg_replace('#<script(.*?)>(.*?)</script>#is', '',$request->get("password")))))->where("is_deleted","0")->get();
   ...
   public function forgotpassword(Request $request){
         $checkmobile=AppUser::where("mob_number",strip_tags(preg_replace('#<script(.*?)>(.*?)</script>#is', '',$request->get("phone"))))->orwhere("email",strip_tags(preg_replace('#<script(.*?)>(.*?)</script>#is', '',$request->get("phone"))))->get();
         if(count($checkmobile)!=0){
  ...
  static public function generate_timezone_list(){
   ...
   public function placeorder(Request $request){
      ...
      if($getuser->profile_pic!=""){
         ...
         $image1=asset('burger/images/web-nav-logo.png');
      ...
      foreach ($cartCollection as $ke) {
        ...
        $adddesc->itemsize=$result["ItemSize"];
      ...
      $addresponse->save();
      Cart::clear();
      event(new FormSubmitted(__('messages.site_name'),__('messages.notificationplaceorder'),$getuser->id,$image1));
      event(new AdminNotification($getuser->name,__('messages.new Order'),'01',$image));
