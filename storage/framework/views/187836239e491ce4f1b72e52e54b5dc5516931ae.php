<style type="text/css">
	
.kb-text-box h1 {

	color: <?=Session::get('combination_color')?> !important;
}

 .first-section .img{
        background-image: url(<?=Session::get("main_banner")?>) !important;
    }

    .footer-section {
      background-image: url(<?=Session::get("footer_img")?>) !important;
   }
   
.kb-text-box h2:after {
	background:<?=Session::get('main_color')?> !important;
	border: 2px solid <?=Session::get('main_color')?> !important;
}

.about-text h5 {
	color: <?=Session::get('heading_color')?> !important;
}

.about-text p {
	color: <?=Session::get('description_color')?> !important;
}

.about-text h5:after {
	background:<?=Session::get('main_color')?> !important;
	border: 2px solid <?=Session::get('main_color')?> !important;
}

.heading h1 {
	color: <?=Session::get('heading_color')?> !important;
}

.tab-b h1 {
	color: <?=Session::get('description_color')?> !important;
}

.head h1 {
	color: <?=Session::get('heading_color')?> !important;
}

.head p {
	color: <?=Session::get('description_color')?> !important;
}

.head h1:after {
	background:<?=Session::get('main_color')?> !important;
	border: 2px solid <?=Session::get('main_color')?> !important;
}

.text-box h2 {
	color: <?=Session::get('heading_color')?> !important;
}

.text-box ul li {
	color: <?=Session::get('description_color')?> !important;
}

.crl {
	background-color: <?=Session::get('main_color')?> !important;
}

.slick-slide .active {
	border: 3px solid <?=Session::get('main_color')?> !important;
}

.size_main_sb .active {
	background-color: <?=Session::get('combination_color')?> !important;
	color: <?=Session::get('white_color')?> !important;
}

.cart a {
	background-color: <?=Session::get('main_color')?> !important;
}

.b-text h1 a {
	color: <?=Session::get('heading_color')?> !important;
}

.b-text p {
	color: <?=Session::get('description_color')?> !important;
}

.size_main_sb a {
	color: <?=Session::get('description_color')?> !important;
}

.price h1 {
	color: <?=Session::get('heading_color')?> !important;
}

.fo-text h1:after {
	background:<?=Session::get('main_color')?> !important;
	border-color: <?=Session::get('main_color')?> !important;
}

.kb-menu ul li a.active {
	border-bottom: 3px solid <?=Session::get('main_color')?> !important;
}

.detail-background-box {
	background-color: <?=Session::get('heading_color')?> !important;
}

.about-heading h2 {
	color: <?=Session::get('heading_color')?> !important;
}

.about-heading p {
	color: <?=Session::get('description_color')?> !important;
}

.about-history h2 {
	color: <?=Session::get('heading_color')?> !important;
}

.about-content-1 p {
	color: <?=Session::get('description_color')?> !important;
}

.about-content-1 p span {
	color: <?=Session::get('main_color')?> !important;
}

.contact-h h1 {
	color: <?=Session::get('heading_color')?> !important;
}

.contact-h p {
	color: <?=Session::get('description_color')?> !important;
}

.contact-head h1 {
	color: <?=Session::get('heading_color')?> !important;
}

.contact-head h1:after {
	background:<?=Session::get('main_color')?> !important;
	border: 2px solid <?=Session::get('main_color')?> !important;
}

.contact-head p {
	color: <?=Session::get('description_color')?> !important;
}

.contact-head .submit {
	background-color: <?=Session::get('main_color')?> !important;
}

.contact-head-1 h1:after {
	background:<?=Session::get('main_color')?> !important;
	border: 2px solid <?=Session::get('main_color')?> !important;
}

.contact-head-1 h1 {
	color: <?=Session::get('heading_color')?> !important;
}

.icons i {
	color: <?=Session::get('main_color')?> !important;
}

.contact-head-1 p {
	color: <?=Session::get('heading_color')?> !important;
}

.login-modal .nav-tabs .nav-link.active, .nav-tabs .nav-item.show .nav-link {
	background-color: <?=Session::get('main_color')?> !important;
}

.modal-login-button button {
	background-color: <?=Session::get('main_color')?> !important;
}

.requiredfield {
	color: <?=Session::get('main_color')?> !important;
}

.nav-pills .nav-link.active, .nav-pills .show>.nav-link {
	background-color: <?=Session::get('combination_color')?> !important;
}

.acc-pro-content h4 {
	color: <?=Session::get('heading_color')?> !important;
}

.acc-pro-content p {
	color: <?=Session::get('description_color')?> !important;
}

.detail-product-box .detail-product-head h4 {
	color: <?=Session::get('main_color')?> !important;
}

.detail-product-box .detail-product-head p {
	color: <?=Session::get('heading_color')?> !important;
}

.detail-product-box .detail-product-content p {
	color: <?=Session::get('description_color')?> !important;
}

.detail-ingredients-head h3 {
	color: <?=Session::get('heading_color')?> !important;
}

.detail-ingredients-head td.selected {
    background: <?=Session::get('main_color')?> !important;
    border: 1px solid <?=Session::get('main_color')?> !important;
    color: <?=Session::get('white_color')?> !important;
}

.detail-ingredients-head table td {
    color: <?=Session::get('heading_color')?> !important;
}

.checkmark {
	border: 1px solid <?=Session::get('heading_color')?> !important;
}

.custom-checkbox-label input:checked ~ .checkmark {
	background-color: <?=Session::get('main_color')?> !important;
    border: 1px solid <?=Session::get('white_color')?> !important;
}

.detail-product-box .detail-plus-add-cart {
	background-color: <?=Session::get('main_color')?> !important;
}

.por-1 .price i {
	color: <?=Session::get('main_color')?> !important;
}

.cric {
	background-color: <?=Session::get('main_color')?> !important;
}

.feature-head h2 {
	color: <?=Session::get('heading_color')?> !important;
}

.feature-head p {
	color: <?=Session::get('description_color')?> !important;
}

.terms-head h4 {
	color: <?=Session::get('heading_color')?> !important;
}

.terms-head ol {
	color: <?=Session::get('description_color')?> !important;
}

.acco-tab-content label span {
	color: <?=Session::get('main_color')?> !important;
}

.acco-c-button button {
	background-color: <?=Session::get('main_color')?> !important;
}

.acco-button .acco-c-button a {
	background-color: <?=Session::get('main_color')?> !important;
}

.acco-button .acco-u-button a {
	background-color: <?=Session::get('combination_color')?> !important;
}

.viewcart h1 a {
	background-color: <?=Session::get('main_color')?> !important;
}

.cart-bill-and-category .cart-head h1 {
	color: <?=Session::get('main_color')?> !important;
}

.cart-category .cart-category-tab {
	color: <?=Session::get('heading_color')?> !important;
}

.cart-category i.fa.fa-trash-o {
	color: <?=Session::get('main_color')?> !important;
}

.cart-category .cart-category-items p {
	color: <?=Session::get('description_color')?> !important;
}

.cart-bill-box .cart-bill-box-head h4 {
	color: <?=Session::get('heading_color')?> !important;
}

.cart-b-subtotal .cart-bs-head h4 {
	color: <?=Session::get('heading_color')?> !important;
}

.cart-b-subtotal .cart-bs-head p {
	color: <?=Session::get('description_color')?> !important;
}

.cart-b-shipping .cart-bs-heading h4 {
	color: <?=Session::get('heading_color')?> !important;
}

.cart-b-shipping .cart-bsc-note p {
	color: <?=Session::get('description_color')?> !important;
}

.checkout-but {
	background-color: <?=Session::get('main_color')?> !important;
}

.checkout-h h1 {
	color: <?=Session::get('heading_color')?> !important;
}

.accordion .card-header {
	border-top: 6px solid <?=Session::get('main_color')?> !important;
}

.accordion.indicator-plus-before.round-indicator .card-header:before {
	color: <?=Session::get('main_color')?> !important;
}

.checkbox-acco1 p {
	color: <?=Session::get('heading_color')?> !important;
}

.checkbox-acco1 a.card-title {
	color: <?=Session::get('combination_color')?> !important;
}

.card-body .sub input {
	background-color: <?=Session::get('main_color')?> !important;
}

.card-body form span {
	color: <?=Session::get('main_color')?> !important;
}

.billing h1 {
	color: <?=Session::get('heading_color')?> !important;
}

#orderplace1 button {
	background-color: <?=Session::get('main_color')?> !important;
}

#orderplacepaypal button {
	background-color: <?=Session::get('main_color')?> !important;
}

#orderplacestrip .stripe-button-el {
	background-color: <?=Session::get('main_color')?> !important;
}

.tab-pane h2 {
	color: <?=Session::get('heading_color')?> !important;
}

.order-nu h1 {
	color: <?=Session::get('heading_color')?> !important;
}

.pending-btn a {
	background-color: <?=Session::get('main_color')?> !important;
}

.order-price h1 {
	color: <?=Session::get('heading_color')?> !important;
}

.order-date h1 {
	color: <?=Session::get('description_color')?> !important;
}

.order-v a:hover {
	background-color: <?=Session::get('combination_color')?> !important;
	color: <?=Session::get('white_color')?> !important;
}

.order-v a {
	color: <?=Session::get('description_color')?> !important;
}

.acco-tab-content label {
	color: <?=Session::get('heading_color')?> !important;
}

.order-d h1 {
	color: <?=Session::get('heading_color')?> !important;
}

.order-d h1:after {
	background:<?=Session::get('main_color')?> !important;
	border: 3px solid <?=Session::get('main_color')?> !important;
}

.order-d-text h1 {
	color: <?=Session::get('heading_color')?> !important;
}

.order-d-text p {
	color: <?=Session::get('main_color')?> !important;
}

.order-status ul .round-d {
	background-color: <?=Session::get('main_color')?> !important;
}

.order-status ul .process-1:after {
	border: 3px solid <?=Session::get('main_color')?> !important;;
}

.main-order-detail h1 {
	color: <?=Session::get('heading_color')?> !important;
}

.main-order-detail p {
	color: <?=Session::get('description_color')?> !important;
}

.main-order-detail span {
	color: <?=Session::get('heading_color')?> !important;
}

.sub-order-text h1 {
	color: <?=Session::get('heading_color')?> !important;
}

.sub-order-text p {
	color: <?=Session::get('description_color')?> !important;
}

.sub-order-text span {
	color: <?=Session::get('heading_color')?> !important;
}

.per-data h1 {
	color: <?=Session::get('heading_color')?> !important;
}

.all-data-order ul li h1 {
	color: <?=Session::get('heading_color')?> !important;
}

.all-data-order ul li span {
	color: <?=Session::get('description_color')?> !important;
}

.sub-total-start h1 {
	color: <?=Session::get('heading_color')?> !important;
}

.sub-total-start span {
	color: <?=Session::get('description_color')?> !important;
}

</style><?php /**PATH C:\xampp\htdocs\project\company\company\scripts\pizzano\pizano\resources\views/user/cssclass.blade.php ENDPATH**/ ?>